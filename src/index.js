/**
 * 云书签 · Cloud Bookmark — Worker 入口
 *
 * 职责：
 *  - /api/*  书签增删改查 + 目标网站元信息（图标 / 标题 / 描述）抓取
...
function collectIconCandidates(html) {
  const links = [];
...
async function tryFetchMetadata(urlStr) {
  try {
...
function ensureSchema(env) {
  if (!schemaReady) {
    schemaReady = env.DB.prepare(SCHEMA_SQL)
      .run()
      .then(() => undefined)
      .catch((err) => {
        schemaReady = null;
...
async function listBookmarks(env) {
...
async function createBookmark(request, env) {
  ...
  const existing = await env.DB.prepare('SELECT id FROM bookmarks WHERE url = ?').bind(url.href).first();
  ...
  const res = await env.DB.prepare(
    'INSERT INTO bookmarks (url, title, description, icon_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
  )
    .bind(record.url, record.title, record.description, record.icon_url, now, now)
    .run();
  return { id: res.meta.last_row_id, ...record, created_at: now, updated_at: now };
}

async function updateBookmark(request, env, id) {
  const body = await readJsonObject(request);
  ...
  if (body.url !== undefined) {
    ...
    url = normalized.href;
    if (url !== row.url) {
      const dup = await env.DB.prepare('SELECT id FROM bookmarks WHERE url = ? AND id != ?').bind(url, id).first();
...
async function handleApi(request, env, url) {
  ...
  if (method === 'GET' && pathname === '/api/bookmarks') {
    return ok(await listBookmarks(env));
  }
  if (method === 'POST' && pathname === '/api/bookmarks') {
    return ok(await createBookmark(request, env));
  }
  ...
  if (match) {
    ...
    if (method === 'PUT' || method === 'PATCH') return ok(await updateBookmark(request, env, id));
    if (method === 'DELETE') return ok(await deleteBookmark(env, id));
  }
