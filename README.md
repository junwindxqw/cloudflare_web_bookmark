...
| 🏷️ | **自动分类** | 新增书签时按内置词典（域名 / 标题关键词）自动归入 18 个分类之一（AI 细分为对话 / 创作 / 开发三类）；提供「一键自动分类」批量处理历史未分类书签；顶部彩色分类 chip 可筛选、带计数，点卡片上的分类徽标同样可筛选 |
...
内置 18 个分类：`tech` 技术、`ai` AI（综合资讯 / 公司 / 研究）、`ai-chat` AI 对话（ChatGPT / Claude / DeepSeek 等）、`ai-image` AI 创作（绘画 / 视频 / 音乐）、`ai-dev` AI 开发（大模型平台 / 框架 / AI 编程）、`design` 设计、`tools` 工具、`news` 新闻、`finance` 财经、`life` 生活、`study` 学习、`shopping` 购物、`video` 视频、`social` 社交、`reading` 阅读、`career` 求职、`cloud` 云服务、`other` 其他。

分类由 `src/category.js` 里的 `CATEGORIES` 数组定义（每项含 `id` / `label` / `color` / `domains` / `weakDomains` / `keywords`）。Worker 在新增 / 编辑 / 「一键自动分类」时按词典匹配，顺序如下：

1. **强域名命中**：先精确匹配再子域后缀匹配（`platform.openai.com` 优先于 `openai.com` 判定），命中即返回。适合域名即身份的站点（如 `github.com` → 技术、`chatgpt.com` → AI 对话）。
2. **关键词加权扫描**：将 `title` 与 `description` 转小写后扫描 `keywords`，**标题命中计 2 分、描述命中计 1 分**（同一关键词以标题命中为准），得分最高的类别胜出。
   - **纯 ASCII 关键词**（如 `api`、`dev`）按 **单词边界** 匹配，避免 `dev` 误中 `developer`、`ai` 误中 `main`。
   - **含中文 / 多字短语** 直接子串包含（CJK 没有词边界概念）。
3. **弱域名兜底（`weakDomains`）**：medium、微信公众号、B 站、知乎等内容平台，域名本身不代表主题——仅当标题 / 描述没有任何关键词信号时，才按 `weakDomains` 兜底归类（如空标题的 B 站链接归入视频，但「B 站上的 Python 教程」由关键词归入学习）。
4. **仍未命中**：落到 `other`。
